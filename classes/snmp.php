...
class snmp {
	...
	function informar_trafego_host($numero_ip,$comunidade,$protocolo) {
		$info=array();
		exec("snmpnetstat -v 1 $numero_ip -c $comunidade -s -P $protocolo -t 0",$info);
	...
	function informar_conexoes_host($numero_ip,$comunidade,$traduz) {
		...
		*/
		...
		$info="";
		if($traduz)
			exec("snmpnetstat -v 1 $numero_ip -c $comunidade -a -t 0",$info);
		else	
			exec("snmpnetstat -v 1 $numero_ip -c $comunidade -an -t 0",$info);
		//transforma o retorno do sistema operacional em um vetor estruturado
		reset($info);
		while(list($chave,$valor)=each($info)) {
			$texto->limpar();
	...
	function informar_rotas_host($numero_ip,$comunidade) {
		...
		$resultado=array();
		$info="";
		exec("snmpnetstat -v 1 $numero_ip -c $comunidade -rn -t 0",$info);
		//transforma o retorno do sistema operacional em um vetor estruturado
		reset($info);
		next($info);
		next($info);
	...
	function informar_interfaces_host($numero_ip,$comunidade) {
		...
		$resultado=array();
		$info="";
		exec("snmpnetstat -v 1 $numero_ip -c $comunidade -i -t 0",$info);
		//transforma o retorno do sistema operacional em um vetor estruturado
		reset($info);
		next($info);
		while(list($chave,$valor)=each($info)) {
	...
	function listar_notificacoes($comunidade) {
		$texto=new texto();
		$piece=array();
		$resultado=array();
		$info="";
		exec("cat /var/log/traps",$info);
		//transforma o retorno do sistema operacional em um vetor estruturado
		reset($info);
		next($info);
		while(list($chave,$valor)=each($info)) {
			...
			//verifica se a notificacao pertence a comunidade solicitada
			if($piece[6]==$comunidade) {
				for($i=0;$i<7;$i++)
					$resultado[]=$piece[$i];

				//cada notificacao e' composta por duas linhas, entao avanca
				list($chave,$valor)=each($info);
				$texto->limpar();
				$texto->adicionar($valor);
				$piece=$texto->dividir(" ");
				...
				//se nao e' a comunidade, avanca ate' a proxima notificacao
				next($info);
							
			//linha em branco entre as notificacoes
			next($info);
		}
		return $resultado;
	}
